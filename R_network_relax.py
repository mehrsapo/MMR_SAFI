...
class RNet(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.W4 = LinearLayer(1, 7, 3, self.n_filters, 1)
        self.W2 = LinearLayer(self.n_filters, 7, 3, self.n_filters, 1)
        self.W3 = LinearLayer(self.n_filters, 7, 3, self.n_filters, 1)

        self.eigenimage =  normalize(torch.nn.Parameter(torch.randn(128, self.n_filters, 40, 40))).detach()
        

        self.phigrad_spline1 = LinearSpline(num_activations=self.n_filters, num_knots=21, x_min=-1, x_max=1, init='zero',
                    slope_max=None, slope_min=None, antisymmetric=False, clamp=False, oneatzero=False)

        self.phigrad_spline2 = LinearSpline(num_activations=self.n_filters, num_knots=21, x_min=-1, x_max=1, init='zero',
                    slope_max=None, slope_min=None, antisymmetric=False, clamp=False, oneatzero=False)
    
        self.phigrad_spline3 = LinearSpline(num_activations=self.n_filters, num_knots=21, x_min=-1, x_max=1, init='zero',
                    slope_max=None, slope_min=None, antisymmetric=False, clamp=False, oneatzero=False)
    ...
    def cal_mask(self, y): 
        self.mask = torch.sigmoid(self.phigrad_spline3(self.W3.W(self.phigrad_spline2(self.W2.W(self.phigrad_spline1(self.W4.W(y)))))))
