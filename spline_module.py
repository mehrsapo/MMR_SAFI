...
class LinearSpline(ABC, nn.Module):
    ...
    def __init__(self, num_activations, num_knots, x_min, x_max, init,
                 slope_max=None, slope_min=None, antisymmetric=False, clamp=True, oneatzero = False, **kwargs):

        super().__init__()
        self.oneatzero = oneatzero
    ...
    def initialize_coeffs(self):
        ...
        if isinstance(init, float):
            ...
            coefficients = torch.sign(grid_tensor)
        elif init == '1mid':
            coefficients = torch.clip(1 - grid_tensor, 0, 1)
        elif init == 'zero':
            coefficients = grid_tensor*0
        elif init == 'one':
            coefficients = grid_tensor*0 + 1
        elif init == 'ccv':
            coefficients = grid_tensor*0 + 1
            coefficients[..., 1:] = coefficients[..., 1:] * 0 
        else:
